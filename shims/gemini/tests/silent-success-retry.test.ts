...
describe("silent success retry integration", () => {
  test("retries a silent Gemini success turn by resuming the same session", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "gemini-cli-shim-fake-"));
...
if (count === 1) {
  writeSession({
    ...
    lastUpdated: "2026-03-09T00:00:01.000Z",
    messages: [
      {
        id: "user-1",
  ...
  emit({ type: "message", timestamp: "2026-03-09T00:00:00.100Z", role: "user", content: "Initial request" });
  emit({
    type: "result",
    timestamp: "2026-03-09T00:00:00.200Z",
    status: "success",
    stats: { total_tokens: 12, input_tokens: 12, output_tokens: 0, cached: 0, duration_ms: 5, tool_calls: 0 },
  });
  process.exit(0);
}
...
emit({
  ...
  tool_id: "write-1",
  parameters: { file_path: "package.json", content: "{}" },
});
emit({
  type: "tool_result",
...
emit({
  ...
  status: "success",
  stats: { total_tokens: 22, input_tokens: 10, output_tokens: 8, cached: 0, duration_ms: 7, tool_calls: 1 },
});
process.exit(0);
`;
    ...
    try {
      ...
      const result = await runCommand(
        ...
        [shimEntry, "--model", "gemini-2.5-flash"],
        {
          ...
          stdin: "Create a file called package.json and then tell me what you created.",
        },
      );
      ...
      expect(
        messages.some(
          (message) =>
            ...
            message.message.content.some((block: Record<string, unknown>) => block.type === "tool_use"),
        ),
      ).toBe(true);
      expect(
        messages.some(
          (message) =>
            ...
            message.message.content.some(
              (block: Record<string, unknown>) =>
                block.type === "text" && String(block.text ?? "").includes("Created the requested files."),
            ),
        ),
      ).toBe(true);
